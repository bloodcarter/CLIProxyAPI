...
This branch tracks stable router-for-me/CLIProxyAPI releases and preserves named standalone Codex inputs, acknowledged synthetic WebSocket warm-up context and startup-ready model routing. Quotio remains the stock macOS app and manages the engine process. These permanent regressions are mandatory before an upstream build is promoted:
...
- `TestServiceRunRegistersLoadedCodexModelsBeforeListenerStart`: keeps loaded OAuth models and their scheduler entries ready before the replacement engine accepts requests, so active Codex turns retry through a restart instead of receiving a terminal startup-only `model_not_found` error.
